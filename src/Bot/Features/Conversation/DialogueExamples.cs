...
public sealed class DialogueExamples
{
    ...
    private static IEnumerable<(DialogueExample Example, int Index)> SelectPack(DialogueExample[] examples) =>
        examples.Select((example, index) => (Example: example, Index: index))
            .Where(x => x.Example.Enabled)
            .OrderByDescending(x => x.Example.Core).ThenBy(x => x.Index).Take(4);

    public IReadOnlyList<AiMessage> BuildMessages(int tokenBudget)
    {
        ...
        foreach (var item in SelectPack(examples))
        {
            ...
            if (TokenEstimate.Count(messages.Concat(block)) > tokenBudget) break;
    ...
    {
        var examples = file.Read();
        // Stable, non-random style demonstrations. Shared words such as "давай"
        // must not retrieve a farewell for a completely different request.
        // The file stays editable: enabled/core and file order determine the pack.
        ...
        foreach (var item in SelectPack(examples))
        {
            ...
            if (TokenEstimate.Count(text.ToString() + block) > tokenBudget) break;
            text.Append(block);
            included.Add(item.Index);
        }
